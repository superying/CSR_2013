function [] = test_features_s2isa(network, params, dim3D)
...
if dim3D
    if(params.num_movies<900)
        [all_train_labels, all_test_labels, all_train_files, all_test_files] ...
            = get_data_summary_3D(params.infopath, params.num_movies);
    else
        [all_train_labels, all_test_labels, all_train_files, all_test_files] ...
            = get_data_summary_3D(params.infopath);
    end
else
   if(params.num_movies<900)
        [all_train_labels, all_test_labels, all_train_files, all_test_files] ...
...
if dim3D
    [Xtrain_raw{1}, MM_train{1}, train_indices{1}] = compute_raw_features_s2isa_3D(network, params, all_train_files, 1); %% made it here
else
...
fprintf('Computing features for all videos in the training set:\n');
if dim3D
    [Xtest_raw{1}, MM_test{1}, test_indices{1}] = compute_raw_features_s2isa_3D(network, params, all_test_files, 1);
else
    [Xtest_raw{1}, MM_test{1}, test_indices{1}] = compute_raw_features_s2isa_3D(network, params, all_test_files, 1);
end
