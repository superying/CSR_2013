function [Xall, MM, indices] = compute_raw_features_s2isa_3D(network, params, data_file_names, size_idx)
...
indices = cell(1); %cell array of size 1

fovea = network.isa{params.feature.num_layers}.fovea; %store fovea from ISA network. Indexed by the number of layers.
...
MM = zeros(3000000, 1, 'single'); %column vector of 3 million zeros of type single

Xall_fill = 1;
% total_time = 0;
for i=1:m   %for all the data files - AH 6/13
%   tic
    
    M = loadclip_3dm([params.avipath{size_idx}, char(data_file_names(i))], fovea.spatial_size, 0, 0, 'RGB'); 
