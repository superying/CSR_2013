...
for i = 1:load_layer_weights
   bases_path_filename = sprintf('%s%s', bases_path, params.bases_id{i}, '.mat'); %MATLAB FUNCTION. Format data into string.
   load(bases_path_filename);   %MATLAB FUNCTION. Load data from MAT-file into workspace

   if iscell(isanetwork) %MATLAB FUNCTION. Determine whether input is a cell array
